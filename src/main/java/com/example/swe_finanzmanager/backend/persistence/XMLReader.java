...
public class XMLReader {
	...
	private DataSet createDataSetFromNode(Element element, ClassType classType) {
		...
		case NUTZER:
			ret = createNutzerDataSet(element);
			break;
		case TRANSAKTION:
			ret = createTransaktionDataSet(element);
			break;
		case KONTO:
			ret = createKontoDataSet(element);
			break;
		}
		return ret;
	}
	...
	private DataSet createKontoDataSet(Element element) {
		KontoDataSet ret = new KontoDataSet(
				BigDecimal.valueOf(Double.valueOf(element.getElementsByTagName("kontostand").item(0).getTextContent())),
	...
	private DataSet createNutzerDataSet(Element element) {
		...
		NutzerDataSet ret = new NutzerDataSet((String) element.getElementsByTagName("vorname").item(0).getTextContent(),
